...
chartA_hue = st.selectbox("Chart A: choose hue variable", columns, index=columns.index("weather") if "weather" in columns else 0)

chartB_x = st.selectbox("Chart B: choose X variable", columns, index=columns.index("location") if "location" in columns else 0)
chartB_hue = st.selectbox("Chart B: choose hue variable", columns, index=columns.index("weather") if "weather" in columns else 0)
...
with col2:
    ...
    sns.countplot(data=df, x=chartB_x, hue=chartB_hue, ax=ax2)
    plt.xticks(rotation=20)
    st.pyplot(fig2)

st.subheader("Random chart test")

if "random_chart" not in st.session_state:
    st.session_state.random_chart = None

if st.button("Show random chart"):
    st.session_state.random_chart = random.choice(["A", "B"])

if st.session_state.random_chart == "A":
    st.subheader("Random Chart: A")
    fig_r, ax_r = plt.subplots()
    sns.countplot(data=df, x=chartA_x, hue=chartA_hue, ax=ax_r)
    plt.xticks(rotation=20)
    st.pyplot(fig_r)

elif st.session_state.random_chart == "B":
    st.subheader("Random Chart: B")
    fig_r, ax_r = plt.subplots()
    sns.countplot(data=df, x=chartB_x, hue=chartB_hue, ax=ax_r)
    plt.xticks(rotation=20)
    st.pyplot(fig_r)

if st.session_state.random_chart is not None:
    st.subheader("Did I answer your question?")
    answer = st.radio("Select:", ["Yes", "No"])
