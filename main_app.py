...
chartA_x = st.selectbox(
    "Chart A: choose X variable",
    columns,
    index=columns.index("location") if "location" in columns else 0
)
chartA_hue = st.selectbox(
    "Chart A: choose hue variable",
    columns,
    index=columns.index("weather") if "weather" in columns else 0
)
chartB_x = st.selectbox(
    "Chart B: choose X variable",
    columns,
    index=columns.index("location") if "location" in columns else 0
)
chartB_hue = st.selectbox(
    "Chart B: choose hue variable",
    columns,
    index=columns.index("weather") if "weather" in columns else 0
)
